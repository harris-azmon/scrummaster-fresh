...
5.  **Select Story:** (same-story or first-incomplete)
    -   **If a story name was provided:**
        ...
        -   **If a unique match is found:** `wiki_read({page:"stories/<story_id>/metadata"})`. If it contains a `depends_on` array, use the `acid_check_dependencies` MCP tool to verify every listed ACID has `accepted` status. If any ACID is not `accepted`, announce which ACIDs are not yet accepted and halt—do not proceed with implementation until those ACIDs are reviewed and set to `accepted` via `scrummaster-review`. **Check the claim** (see step 6 below). If it's held by another agent and not stale, this is very often just a resume from an earlier session rather than a real conflict (the claim's `agent_id` is per-run, not a persistent identity, so a new session on the same story always looks like "another agent" here) — announce the existing claim (who holds it, when it was made) and ask a **Yes/No question**: proceed anyway and take over the claim, or stop (do not silently HALT here; that would force a manual `scrummaster-revert` just to resume your own work). Once the story is confirmed available (unclaimed, self-claimed, stale, or the collision override was accepted), ask the normal **Yes/No** confirmation to proceed with implementing it.
    ...
    -   **If no story name was provided:**
        -   **Identify Next Story:** Walk incomplete stories in registry order. For each candidate, `wiki_read({page:"stories/<story_id>/metadata"})`; if it contains a `depends_on` array, use `acid_check_dependencies` and skip this candidate (continue to the next) if any listed ACID is not `accepted`. **Check the claim** (step 6) — if held by another agent and not stale, skip this candidate too and continue to the next (don't interrupt the scan for every claimed story while other unclaimed candidates might exist).
        -   **If a viable (unclaimed, or self-claimed) candidate is found:** Propose it to the user and ask for confirmation (Yes/No).
        -   **If none found because all remaining incomplete stories are `[x]`-adjacent or dependency-blocked:** Announce that and HALT.
        -   **If none found solely because every remaining incomplete story is claimed by another (non-stale) agent:** Don't flatly HALT — this is the same likely-a-resume situation as the named-story case above (especially common when there's exactly one `[~]` story left over from an earlier session). Announce the first such candidate's existing claim and ask a **Yes/No question**: proceed anyway and take over the claim, or stop.
6.  **Claim Check (used by step 5):** Given a candidate story's metadata, decide:
    ...
    -   **Claimed by another agent, not stale** (or `claim_lease_hours` unconfigured — claims never auto-expire): **not available**, unless the user explicitly overrides (see §2 step 5's Yes/No override for the collision case).
    -   **Missing `claimed_by`/`claimed_at` keys entirely** (metadata written before this feature existed): treat identically to explicit `null` — unclaimed, available.

## 3. Story Implementation

1.  **Announce Action:** Announce which story you are beginning to implement.
2.  **Update Status to 'In Progress':**
    -   **Re-check, claim, and write metadata immediately — before anything else:** `wiki_read({page:"stories/<story_id>/metadata"})` fresh (don't reuse the read from §2 — time has passed for user confirmation, and this narrows the collision window as much as this best-effort scheme can). Re-apply the Claim Check (§2 step 6). If it now reads "not available" and the user hasn't already overridden it in §2, announce the collision and HALT — do not proceed or overwrite their claim. Otherwise, in this same step, set `claimed_by` to this run's `agent_id`, `claimed_at` to `now_iso()`, and `ready_entered_at` to `now_iso()` on the metadata object, and `wiki_write` it back (mimetype `plain`) **right now** — do not defer this write until after the registry update below. The whole point of re-checking "right before the state change" is defeated if another wiki round-trip (the registry flip) sits between the read and the claim write; keep that gap to a single read+write pair.
    -   Only after the metadata claim write above has landed: edit the `stories` registry page's text to flip this story's marker to `[~]`, then `wiki_write({page:"stories", content, mimetype:"markdown"})`.
