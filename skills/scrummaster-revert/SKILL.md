...
2.  **Reset Plan State:** `wiki_read` the relevant `stories/<id>/plan` (and `stories` if reverting a whole story), reset the reverted tasks back to `[ ]` (pending) or `[~]` as appropriate, `wiki_write` it back. Optionally reset the corresponding ACID ticket status via the `acid_set_status` MCP tool. **If reverting a whole story back to `[ ]`:** also `wiki_read({page:"stories/<id>/metadata"})`, set `claimed_by` and `claimed_at` to `null`, and `wiki_write` it back — the story is fully unclaimed again so another (or the same) agent can pick it up cleanly, rather than leaving a stale claim from the aborted attempt.
