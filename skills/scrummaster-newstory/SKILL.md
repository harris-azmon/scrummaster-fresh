...
5.1 **Stamp Commit Timestamps:** Immediately before the Fossil commit in step 8, set `spec_committed_at` and `plan_committed_at` in `metadata.json` to `now_iso()` (they will be identical here since spec.md and plan.md are written and committed together — that's fine, they're still the anchor for later drill-down if the flow ever splits the two commits).
