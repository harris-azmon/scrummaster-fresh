"""Determines the next unblocked setup step in the Scrummaster workflow."""
...
def determine_resumption():
  ...
  for filename in files:
    path = os.path.join(scrummaster_dir, filename)
    checklist[filename] = os.path.exists(path)

  setup_complete = os.path.exists(os.path.join(scrummaster_dir, "index.md"))
