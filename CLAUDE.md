...
- **`agents/*.md`** — OpenCode `mode: subagent` definitions for **Turbo Mode** (`scrummaster-product-manager`, `scrummaster-software-architect`). Opt-in, off by default via a `Turbo Mode` marker on the `workflow` wiki page (set during `scrummaster-setup`). Every `SKILL.md`'s "Operational Standards" section carries an identical "Turbo Mode" bullet right after "Sequential Questioning (CRITICAL)" — when Turbo Mode is enabled, that bullet redirects every question the skill would present to the human to one of these two subagents instead, and the subagent's answer is treated exactly as the human's. These agents are not registered anywhere else in the repo's own config (`opencode.json` has no `agents.paths` key — OpenCode discovers them only from fixed directories, `.opencode/agents/` project-local or `~/.config/opencode/agents/` global); the README's install section symlinks `agents/*.md` into one of those.
...
4. **`scrummaster-implement`** — before pulling a story, checks configured `ready_wip_limit`/`review_wip_limit` (from `workflow`'s Flow Control section) and backpressures if a lane is full. Picks a story (respecting `depends_on` gating via `acid_check_dependencies` — dependencies must be `accepted` first, and a per-story **claim** — see below), executes plan tasks in order per `workflow`, committing real code changes after each task (`fossil_add`/`fossil_commit` — its result's `hash`/`hash_short` is written straight into the plan page, no separate hash lookup), and flips `[x]` markers via `wiki_write` as the actual driver of progress. On story completion it optionally syncs the `product`/`tech-stack`/`product-guidelines` pages.
...
## Multi-agent safety: WIP limits vs. claims

Two different problems, two different mechanisms — don't conflate them:
- **Too many stories in flight at once** (across different stories): `ready_wip_limit`/`review_wip_limit` (`workflow`'s Flow Control section, §2 of `scrummaster-implement`). Just a count check against the registry.
- **Two agents picking the *same* story**: solved by a claim/lease pair on each story's metadata (`claimed_by`, `claimed_at`), checked and set by `scrummaster-implement` (§2 step 6 / §3 step 2) and cleared by `scrummaster-revert` when a whole story is reset. This is deliberately **not** a real lock — Fossil wiki writes aren't compare-and-swap, so there's an unavoidable (if narrow) TOCTOU race between the pre-check and the write. `claim_lease_hours` (optional) lets a stale claim from a crashed/abandoned run expire automatically instead of blocking the story forever.

Branch-per-story isolation was considered as an alternative/complement to WIP limits and rejected: this repo is explicitly Cathedral-style/trunk-only (see `README.md`) with no branch-creation step anywhere in any skill, and introducing one would be a much larger philosophy change than the problem (duplicate story selection) actually requires — the claim mechanism solves that directly without touching the trunk-only model.

## Conventions when editing `SKILL.md` files

- Every skill's "Operational Standards" section repeats the same rules (precise execution, validate every tool call, `wiki_*`/`fossil_*`/`acid_*` MCP tools only — never a raw shell `fossil` command or a guessed filesystem path, sequential single-choice/Yes-No/multiple-choice questioning with a preferred option marked `(Recommended)` and an "Other" fallback, and a Turbo Mode bullet redirecting those same questions to a subagent when enabled). Keep new/edited protocol steps consistent with that interaction style rather than introducing open-ended free-text prompts.
